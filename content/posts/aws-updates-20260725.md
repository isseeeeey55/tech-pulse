...
draft: true
tags: ["aws", "kinesis", "ec2", "lambda", "ses", "bedrock", "cloudwatch", "alb", "s3", "data-firehose", "license-manager", "step-functions"]
...
今回のアップデートにより、ストリームのライトスループットを低い値に設定すると、Kinesisは自動的に**その値と過去1時間のピークトラフィックに必要な容量のいずれか高い方**に調整します。これにより、現在のトラフィックに必要な容量は確保しつつ、不要な過剰容量を自動的に解放できるようになりました。

#### 検証のポイント

この機能を活用する際の検証ポイントは以下の通りです。

1. **On-demand Advantageモードの有効化**: まず既存のストリームをOn-demand Advantageモードに切り替えます。この機能は追加費用なしで利用できるため、既存のオンデマンドストリームで有効にするだけで利用開始できます。
...
4. **コスト削減効果の定量化**: スケールアップのみの運用と比較して、実際のコスト削減効果を測定します。特にトラフィックの変動パターンが明確なワークロード（ECサイトのアクセス変動、IoTデバイスからの周期的なデータ送信など）では、顕著なコスト削減が期待できます。

#### 従来モードとの比較

Kinesisには現在、**Provisionedモード**、**On-demandモード**、**On-demand Advantageモード**の3つの容量モードが存在します。Provisionedモードは完全に手動でシャード数を管理する必要があり、従来のOn-demandモードはスケールアップのみ対応していました。今回のOn-demand Advantageモードは、スケールアップとスケールダウンの両方に対応し、トラフィック変動が激しい環境で最もコスト効率の良い選択肢となります。
...
ALBログのCloudWatch Logs配信は、vended logsとして課金されます。また、Amazon Data Firehose経由での配信や、Amazon S3への配信（Apache Parquet形式に対応）もサポートされます。S3への配信自体は無料ですが、Parquet変換には$0.035/GB（バージニア北部リージョン）のコストがかかります。

従来のS3ログ配信と比較した場合、CloudWatch Logsはリアルタイム性と分析の容易さで優位性がありますが、長期保存のコストはS3の方が低くなります。一般的には、リアルタイム分析用にCloudWatch Logs、長期アーカイブ用にS3という組み合わせが推奨されます。
...
導入時の判断基準としては、まずトラフィックパターンの分析が重要です。CloudWatchメトリクスで過去1〜3ヶ月のトラフィック推移を確認し、ピークと通常時の差が大きい場合（例：2倍以上の差）は、スケールダウン機能の効果が期待できます。ただし、過去1時間のピークトラフィック判定ロジックがあるため、1時間以内に急激なトラフィック増加が発生するパターンでは、warm throughput設定を慎重に調整する必要があります。

Terraformでインフラを管理している環境では、Kinesis Streamリソースの設定にOn-demand Advantageモード有効化を含めることで、新規ストリームでも自動的にこの機能を利用できます。また、CloudWatch Alarmsでスケーリングイベントやコンシューマーのバックログを監視し、スケールダウンが意図しないパフォーマンス低下を引き起こしていないかを継続的に確認することが推奨されます。
...
AI/ML領域では、Claude Opus 5の追加と aws-bench の公開により、生成AIワークロードの品質評価と標準化が進んでいます。特に aws-bench は、AIエージェントの信頼性を客観的に測定する基盤を提供し、今後のエンタープライズAI採用において重要な役割を果たすでしょう。

SRE業務においては、これらのアップデートを段階的に評価し、既存の監視・運用プロセスに統合していくことが推奨されます。特にCloudWatch Logs統合やKinesis自動スケーリングは、PoC環境での検証を経て本番環境に適用することで、運用負荷の軽減とコスト最適化の両立が期待できます。
