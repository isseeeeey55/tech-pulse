...
Claude Code v2.1.187 がリリースされました。今回のリリースでは、セキュリティ強化を目的とした認証情報アクセス制限機能、組織レベルのモデル制限設定、構造化出力の安定性向上など、合計 20 件の変更が行われています。特にサンドボックス環境のセキュリティ強化と、リモート実行時の堅牢性向上が主要なテーマとなっています。
