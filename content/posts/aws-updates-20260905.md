...
![](/images/aws-updates-20260905/header.png)

# 直近発表のAWSアップデート紹介：ECSデプロイメント改善とGraviton5インスタンス拡大

## はじめに

今回は、直近で発表された11件のAWSアップデートを紹介します。ECSのデプロイメント完了判定が今回から柔軟化され、Early Success Criteriaや非クリティカルデーモンといった柔軟な運用を支える機能が追加された点が注目ポイントです。また、AWS Graviton5プロセッサを搭載したM9g/C9gインスタンスが東京を含むアジア太平洋リージョンで続々と利用可能になり、最大35%の性能向上を実現しています。さらに、AI/MLワークロード向けにP6シリーズのGPUインスタンスがジャカルタやハイデラバードで展開され、グローバルな推論・学習基盤の選択肢が広がりました。AWS Transfer FamilyやMCP Serverといった運用・開発支援ツールの機能強化も含め、エンタープライズ運用の安定性とパフォーマンスを両立するアップデートが揃っています。
...
東京リージョンでM9g/C9gが利用可能になったことで、日本国内の低レイテンシ要件を満たしつつ、コストとパフォーマンスを最適化できます。特に以下のようなワークロードでの活用が見込まれます：
...
> **EFA（Elastic Fabric Adapter）とは？** AWSが提供するクラスター向け高性能ネットワークインターフェースです。v4では帯域が3.2 Tbpsに拡大され、分散学習でのGPU間通信を高速化します。P6インスタンスに搭載されています。
...
今回紹介したアップデートは、大きく3つのテーマに分けられます。第一に、ECSのデプロイメント機能強化（Early Success Criteria、非クリティカルデーモン）により、コンテナベースのマイクロサービス運用がより柔軟で堅牢になりました。第二に、Graviton5（M9g/C9g）インスタンスの東京リージョン展開により、日本国内のワークロードで高いコストパフォーマンスと低レイテンシを両立できる環境が整いました。第三に、ジャカルタ・ハイデラバードへの展開で、アジア太平洋での大規模AI学習がより低レイテンシで実行できるようになりました。

全体として、運用の安定性向上、パフォーマンスとコスト最適化、グローバル展開の柔軟性といった、エンタープライズのニーズに応えるアップデートが並んでいます。どれも段階的に試せる変更です。まずECSのEarly Success Criteriaを追加し、CI/CDの待ち時間への影響を実測するところから始めると無駄がありません。
