...
この修正により、サブエージェントが誤ってメインリポジトリに影響を与えるリスクが排除されます。ワークツリー隔離機能は、複数の並行タスクを安全に実行するために設計されており、この修正はその設計意図通りの動作を保証します。
...
| Feature | Fable アドバイザー一時非表示 | Fable アドバイザーの障害を引き起こすサーバー側問題の修正中、アドバイザーピッカーで Fable が一時的に利用不可として表示 |
