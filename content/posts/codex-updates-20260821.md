...
> **Note:** Codex CLI では「エージェント」「タスク」「セッション」といった用語が、ユーザーとの対話型セッション単位を指す場合があります。`codex agents` は、これらのセッション全体を統合管理するためのインターフェースです。
...
```bash
$ codex queue <session-name> "追加で実装してほしい機能の詳細"
```

このコマンドは、指定したセッションにメッセージを送り、アイドル状態のセッションを起動させます。キューイングされたメッセージは、セッション名の重複解決がより有用になり、ペーストや遅延コマンドのセマンティクスも保持されるようになりました（#39034, #39385, #39604）。これにより、SRE が監視スクリプトを実行しながら、別ターミナルから追加の解析指示を送信する、といった柔軟な運用が可能になります。
...
従来、TUI セッション内でディレクトリコンテキストを変更するには、別途シェルコマンドを実行するか、セッションを再起動する必要がありました。この機能追加により、複数のプロジェクトディレクトリを横断しながら作業する場合でも、セッションを維持したままシームレスにディレクトリを切り替えられます。インフラコード（Terraform、Ansible など）を複数のディレクトリで管理している SRE にとって、作業効率が大きく向上します。
...
これらのコマンドは TUI セッション内で直接入力でき、セッションのコンテキストを保持したまま作業ディレクトリを変更できます。
