...
Claude Code v2.1.202 がリリースされました。このバージョンでは、動的ワークフローのサイズ制御設定の追加、OpenTelemetry テレメトリ属性の追加、および 15 件の不具合修正が含まれています。主な修正対象は、インライン履歴検索のクラッシュ、セッションリネームの永続化、mTLS ハンドシェイク失敗、リモートコントロールからのコマンド実行、音声入力の無限ループなど、セッション管理とリモート操作周りの信頼性向上に重点が置かれています。
...
| Breaking | `/review <pr>` の挙動変更 | 高速シングルパスレビューに戻す；マルチエージェントレビューは `/code-review <level> <pr#>` で実行 |
