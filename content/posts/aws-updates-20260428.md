...
管理者は Amazon Connect 管理画面または API を通じて、この新機能を有効化できます。以下は AWS CLI を使用した設定例です：

```bash
# ファイル添付の上限設定を確認
$ aws connect describe-instance-attribute \
  --instance-id your-instance-id \
  --attribute-type ATTACHMENT_FILE_SIZE_LIMIT

# カスタムファイル拡張子を設定
$ aws connect update-instance-attribute \
  --instance-id your-instance-id \
  --attribute-type ALLOWED_FILE_EXTENSIONS \
  --value '{"AllowedExtensions": [".dicom", ".hl7", ".dwg", ".dxf", ".log", ".tar.gz"]}'
```

実際の検証では、以下のステップでパフォーマンスと動作を確認することをお勧めします：

1. **アップロード速度の計測**：50MB、75MB、100MBの各サイズでファイルをアップロードし、所要時間を記録
2. **カスタム拡張子の動作確認**：業界固有のファイルフォーマット（.dicom、.hl7、.dwg など）をアップロード
3. **チャット、ケース、タスク各チャネルでの挙動検証**：それぞれのインターフェースで100MBファイルの送受信が正常に動作するか確認
4. **エラーハンドリングの確認**：上限を超えたファイルや許可されていない拡張子をアップロードした際のエラーメッセージを検証

#### 業界別の活用例

**テクノロジー企業**では、システムログやデバッグ情報を含む診断バンドルを直接チャットで共有でき、サポート対応の初回解決率（FCR）向上が期待できます。

**金融サービス**では、署名済み契約書やコンプライアンス文書をカスタム拡張子で受け付けることで、顧客が直接チャットやメールで提出できる仕組みを構築できます。

**ヘルスケア業界**では、医療画像（DICOM形式）や患者記録（HL7形式）を安全に添付し、HIPAA準拠を維持しながら効率的な情報共有が可能になります。

**製造業**では、製造図面や仕様書、品質チェックレポート（PDF、CADファイル）の直接共有により、製品不具合の早期解決が実現します。

#### セキュリティとコストの考慮点

ファイルサイズの拡大に伴い、以下のセキュリティ対策を検討する必要があります：

- ウイルススキャンの実装とスキャン時間の許容範囲確認
- ファイルタイプの検証とホワイトリスト方式の採用
- ストレージコストの増加予測と監視体制の構築
