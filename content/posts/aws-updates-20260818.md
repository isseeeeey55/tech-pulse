...
この機能自体に追加料金は発生しませんが、CloudWatchメトリクスのストレージとクエリには標準料金が適用されます。大規模環境では、メトリクス保存期間を適切に設定し、不要な長期保存を避けることでコストを抑制できます。
...
**Geoモード（US Geo対応）**  
特定の地理的領域内でのみ推論処理を実行します。GDPRやデータレジデンシー規制がある環境では、このモードを選択することで、コンプライアンス要件を満たしながらマルチリージョンの恩恵を受けられます。

#### APIの統合と使い分け

Bedrock-runtimeエンドポイント上で、以下の3つのAPIが利用可能になります：

- **Responses API**: ストリーミングレスポンスに対応した新しいAPI
- **Converse API**: 会話型アプリケーション向けのシンプルなインターフェース
- **Chat Completions API**: OpenAIのAPI仕様と互換性のあるエンドポイント

Chat Completions APIの提供により、既存のOpenAI統合コードをほぼそのまま移行できるため、ベンダーロックインのリスクを軽減しながらAWS環境でOpenAIモデルを活用できます。

#### CloudWatchとCost Explorerによる統合管理

推論メトリクス（呼び出し回数、トークン数、レイテンシ）はCloudWatchに自動的に記録されます。これにより、既存のAWS監視基盤に統合し、一元的なダッシュボードで管理できます。

Cost Explorerでは、リージョンごと、モデルごとの推論コストを詳細に確認できます。グローバルモードを使用している場合、どのリージョンで何回推論が実行されたかを追跡し、コスト最適化の判断材料として活用できます。
...
例えば、500台のインスタンスを終了する場合を考えます：

**従来の方法**  
- API呼び出し回数: 500回
- API呼び出し間のオーバーヘッド: 約100〜200ms/回
- レート制限による待機時間: 場合により数分
- 合計スケールダウン時間: 5〜10分
...
これにより、スケールダウン時間が大幅に短縮され、不要なインスタンス稼働時間によるコストを削減できます。
...
コスト最適化の観点では、スケールダウン時間の短縮により、不要なインスタンス稼働時間を削減できます。例えば、深夜バッチ処理が完了してから全インスタンスが終了するまでの時間が5分短縮されれば、月間で相当なコスト削減効果が期待できます。Cost Explorerで実際の削減額を可視化し、チーム内で共有することで、継続的な改善のモチベーションにつながります。
...
| Amazon EC2 | [R8i/R8i-flexインスタンス カルガリーリージョン対応](https://aws.amazon.com/about-aws/whats-new/2026/08/amazon-ec2-r8i-r8i-flex-calgary/) | Intel Xeon 6搭載のメモリ最適化インスタンス。R7iと比べ20%高性能、2.5倍のメモリ帯域幅 |
...
Amazon Bedrockのクロスリージョン推論は、生成AIアプリケーションのスケーラビリティと可用性を大きく向上させる一方で、データレジデンシー要件への対応も同時に実現しています。これは、グローバル展開とコンプライアンス要件の両立という、現代のクラウドアーキテクチャが直面する課題への明確な回答と言えます。
