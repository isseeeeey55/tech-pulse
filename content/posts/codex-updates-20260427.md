...
パーミッションプロファイルがセッション間で保持されるようになったことで、Codex CLI の利用体験が大幅に向上しました。以前は、各セッションごとにパーミッション設定を再構築する必要がありましたが、今回の変更により、一度設定したプロファイルが複数のセッションにまたがって有効になります。

> **Note:** パーミッションプロファイルは、Codex CLI が実行可能な操作の範囲を定義する設定で、approval mode と連携して動作します。

この変更は、チーム開発や複数プロジェクトを横断する作業において特に効果的です。プロジェクトごとに異なるセキュリティポリシーを適用しながらも、設定の再入力を避けることができます。また、組織全体で統一されたパーミッション基準を維持しやすくなり、セキュリティコンプライアンスの観点からも望ましい改善です。
...
| Improvement | ロールアウト追跡の強化 | 機能展開の監視と管理の向上 |
