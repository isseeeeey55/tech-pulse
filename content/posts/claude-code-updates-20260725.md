...
![](/images/claude-code-updates-20260725/header.png)
...
Claude Opus 5 の 1M コンテキスト長により、大規模なコードベースやドキュメントを一度に処理できるようになります。fast モードの料金（入力 $10 / 出力 $50 per Mtok）も明示されているため、通常モードとの使い分けを判断しやすくなっています。
...
v2.1.219 は、Claude Opus 5 という新しいフラッグシップモデルの導入を中心に、27 項目の変更が入ったアップデートです。1M コンテキスト長による大規模処理能力の向上に加え、サンドボックスネットワークの厳密な制御、MCP サーバーエラーの可視性向上、セルフホストランナーの安定性改善など、開発体験と信頼性の両面で変更が入っています。動的ワークフローのサイズガイドラインがデフォルト化され、サブエージェントのネスト深度も拡張されたことで、ワークフロー構築の選択肢が広がりました。Vim モードやスクリーンリーダー、GNU screen での挙動といった細かな UI/UX の修正も含まれています。
