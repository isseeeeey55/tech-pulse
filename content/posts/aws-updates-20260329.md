...
2026年3月29日のAWSアップデートは2件。CloudWatch Logs の低頻度アクセス（IA）クラスに OpenSearch PPL/SQL とデータ保護機能が追加されたことと、Amazon Timestream for InfluxDB に高度なメトリクス機能が入ったこと。どちらもコスト最適化と運用監視に直結するアップデートです。
...
Amazon CloudWatch Logsの低頻度アクセス（IA）クラスに、データ保護機能と OpenSearch PPL/SQL サポートが追加された。これまで標準ログクラスでしか使えなかった分析・保護機能が、IAクラスでも利用可能に。

**IAクラスの制約が緩和された**

Logs IAクラスは取り込み料金が標準クラスの50%で済む反面、クエリ機能に制限があった。今回の追加で、セキュリティ監査やコンプライアンス対応のログ分析をIAクラスのままで実行できるようになった。
...
InfluxDB 2インスタンスの詳細な運用メトリクスが、追加設定なしで自動的にCloudWatchへ送信されるようになった。

**何が変わるか**

InfluxDBはセンサーデータやメトリクスを大量に処理するため、DB自体の健全性監視が欠かせない。これまでは自前でメトリクス収集の仕組みを用意する必要があったが、有効化するだけでCloudWatchに流れるようになる。
...
月次レポートや四半期レビューでしか参照しないログは、IAクラスへの移行候補になる。Terraform でログストリームの `log_class` を `INFREQUENT_ACCESS` に変えるだけで取り込みコストが半減し、PPL/SQLでの分析もデータ保護も使える。PCI DSS や GDPR 対応のログも対象にできる。

ただし、リアルタイムアラートが必要なログは標準クラスのまま残すこと。判断基準は「そのログに即時性が求められるかどうか」。

**Timestream for InfluxDB メトリクスの使いどころ**

CloudWatch アラームと組み合わせれば、InfluxDB の異常を早期検知してランブックに組み込める。これまで自前で構築していた監視パイプラインが不要になるのは地味に効く。

注意点として、メトリクス送信分の CloudWatch 料金が加算される。不要なメトリクスは送信対象から外す設定を入れておくといい。
...
2件ともコスト最適化と運用監視がテーマ。CloudWatch Logs IA は「安いけど機能が足りない」という従来の弱点が解消され、より多くのログをIAクラスに寄せられるようになった。Timestream for InfluxDB の高度なメトリクスは、有効化するだけで CloudWatch に流れるので導入コストが低い。週明けにでも設定を見直してみるといい。