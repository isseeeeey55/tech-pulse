...
![](/images/aws-updates-20260527/header.png)

# 2026年5月27日 AWS アップデート情報

## はじめに

2026年5月27日は、**6件**のAWSアップデートが公開されました。中でも注目すべきは、**Amazon EC2の新世代インスタンス（M8i/M8i-flex、R8i/R8i-flex）がAWS GovCloud (US-East)リージョンで利用可能になったこと**、そして**Amazon VPC IPAMがプール割り当てへのタグ付けに対応**し、大規模マルチアカウント環境でのIP管理ガバナンスを強化しやすくなったことです。また、**Amazon RDSがMulti-AZレプリケーションでENA Expressに対応**し、書き込みスループットとレイテンシの改善が期待できるようになりました。本記事では、これらのアップデートを技術的に深掘りし、SRE視点での活用方法を解説します。

---

## 注目アップデート深掘り

### Amazon EC2 M8i/R8i 世代インスタンスのGovCloud対応

Amazon EC2の**M8i/M8i-flex**および**R8i/R8i-flex**インスタンスが、AWS GovCloud (US-East)リージョンで利用可能になりました。いずれもAWS独自のカスタムIntel Xeon 6プロセッサを搭載し、前世代と比較して性能とメモリ帯域幅の改善が示されています。

#### なぜこのアップデートが重要なのか

GovCloudは米国政府機関や規制対象業界向けに設計されたリージョンであり、最新世代の汎用・メモリ最適化インスタンスが利用可能になることで、コンプライアンス要件を満たしながら性能改善を取り込めるようになります。特に注目すべきは以下の改善です：

- **前世代比で価格性能比が最大15%向上**
- **メモリ帯域幅が2.5倍に向上**
- **PostgreSQLで最大30%高速化、NGINXウェブアプリケーションで最大60%高速化、AI深層学習推奨モデルで最大40%高速化**

#### M8i/R8iとFlexの使い分け

**M8i/M8i-flex**は汎用ワークロード向けです。M8i-flexはlargeから16xlargeまでのよく使われるサイズを提供し、常に全リソースを使い切るわけではないWeb/API、マイクロサービス、小中規模データストアに向いています。一方、M8iは最大96xlargeまでスケールでき、SAP認定も受けているため、継続的に高いCPU使用率が見込まれるエンタープライズアプリケーションに適しています。

**R8i/R8i-flex**はメモリ最適化ワークロード向けです。R8i-flexはメモリ集約型ワークロードでFlexの価格性能メリットを得たい場合の選択肢で、R8iは大規模インメモリデータベースやSAPなど、より大きなサイズや継続的な高CPU使用率が必要な用途に向きます。R8iはSAP認定に加え、公式発表で142,100 aSAPSの性能値も示されています。

従来のM7i/R7i系インスタンスを使用している場合、特にデータベースやWebアプリケーションのワークロードでは、以下のような改善が期待できます：
...
GovCloud環境では、コンプライアンス要件に応じたタグ付けが重要です。上記の例では、環境やワークロードを識別するタグを付与しています。メモリ最適化ワークロードを移行する場合は、`r8i.xlarge` や `r8i-flex.xlarge` のようにインスタンスタイプを切り替えて検証します。
...
既存のRDSインスタンスでENA Expressを有効化するには、公式発表では**start-stopまたはコンピュートスケーリングアクション**が必要とされています。以下は、AWS CLIを使用してインスタンスを停止・起動することで有効化を促す手順です：
...
# インスタンスを起動（対応インスタンスであれば、この操作後にENA Expressが適用される）
$ aws rds start-db-instance \
    --db-instance-identifier my-production-db
```

> **Note:** インスタンスの停止・起動によるダウンタイムが発生します。また、ENA Expressの対象インスタンスタイプに含まれるかを、事前にRDSユーザーガイドで確認してください。
...
### M8i/R8i世代インスタンスの運用シナリオ

Terraformで管理しているインフラがあれば、M8i/M8i-flexやR8i/R8i-flexへの移行は比較的検証しやすい施策です。特に、PostgreSQLやNGINXを使用しているシステムでは、インスタンスタイプの変更だけで性能改善が期待できるため、GovCloud環境でのコスト対効果を測る候補になります。

導入時の判断基準としては、以下を検討すると良いでしょう：

- **ワークロードの特性**: Web/APIや小中規模データストアならM8i系、メモリ集約型データベースやSAPならR8i系など、性能改善の恩恵を受けやすいワークロードか
- **リージョン制約**: GovCloudでの運用が必要か、または他のリージョンでの展開を待つか
- **サイズ選択**: 可変的なワークロードにはFlex、大規模または継続的な高CPU使用率のワークロードには通常のM8i/R8iを選択
